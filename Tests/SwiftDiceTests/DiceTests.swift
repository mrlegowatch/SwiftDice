...
struct DiceTests {
    ...
    @Test("Division by zero modifier returns zero")
    func divisionByZeroModifier() {
        let dice = Dice.d6 / 0
        #expect(dice.roll().result == 0)
    }

    @Test("Dropping all dice is clamped to no-drop")
    func droppingAllDiceClamped() {
        let dice = Dice.d6.dropping(.lowest)
        #expect(dice.selection.count == 0)
        rollSample(dice, in: 1...6)
    }
