...
struct DiceParserTests {
    ...
    func negativeModifier() throws {
        let dice = try parser.parse("-3")
        #expect(dice.description == "-3")
    ...
    func operatorPrecedenceMultiplyBeforeAdd() throws {
        let dice = try parser.parse("1+2*3")
        #expect(dice.description == "1+2x3")
        #expect(dice.roll().result == 7)  // 1 + (2*3) = 7, not (1+2)*3 = 9
    }

    @Test("Operator precedence with dice")
    func operatorPrecedenceWithDice() throws {
        let dice = try parser.parse("1+d6*2")
        #expect(dice.description == "1+d6x2")
        rollSample(dice, in: 3...13)  // 1 + (1..6)*2 = 3..13, not (1+1..6)*2 = 4..14
    }

    @Test("Parentheses grouping with constants")
    func parenthesesGroupingConstants() throws {
        let dice = try parser.parse("(2+3)*4")
        #expect(dice.roll().result == 20)  // (2+3)*4 = 20, not 2+(3*4) = 14
    }

    @Test("Parentheses grouping with dice")
    func parenthesesGroupingWithDice() throws {
        let dice = try parser.parse("(d6+2)*3")
        rollSample(dice, in: 9...24)  // (1..6+2)*3 = 9..24
    }
    ...
    func invalidDiceFormatStrings(badFormatString: String, reason: String) {
        #expect(throws: DiceParseFailure.self, "'\(badFormatString)' \(reason)") {
            _ = try parser.parse(badFormatString)
        }
    }

    @Test("Parse failure carries input and caret position")
    func parseFailureDiagnostic() {
        do {
            _ = try parser.parse("2d4H")
            Issue.record("Expected DiceParseFailure")
        } catch let failure as DiceParseFailure {
            #expect(failure.error == .missingMinus)
            #expect(failure.input == "2d4H")
            #expect(failure.offset == 3)
            let desc = failure.errorDescription ?? ""
            #expect(desc.contains("2d4H"))
            #expect(desc.contains("   ^"))
        } catch {
            Issue.record("Expected DiceParseFailure, got \(type(of: error)): \(error)")
        }
    }
