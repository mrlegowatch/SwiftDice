...
`DiceParser` converts a dice notation string into a `Rollable` instance. It throws `DiceParseFailure` if the expression cannot be parsed.
...
`DiceParseFailure` wraps a `DiceParseError` with the original input string and the Unicode scalar offset of the problem, producing a compiler-style caret diagnostic via `errorDescription`:

```swift
do {
    let roll = try parser.parse("2d4H")
} catch let failure as DiceParseFailure {
    print(failure.error)              // missingMinus
    print(failure.errorDescription!)
    // A drop modifier requires a preceding '-' (e.g. "4d6-L")
    // 2d4H
    //    ^
}
```
